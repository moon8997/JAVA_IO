			...
			try {
				 ...
				 if(srcFileSize > maxSize) {
					...
					// 원본 파일의 크기가 10Mb 를 초과하지 않은 경우
					 ...
					 // bist[필터스트림(보조스트림)]의 버퍼크기는 1024*1024byte 로 했다.
					 ...
					 BufferedOutputStream bost = new BufferedOutputStream(fost, cnt);
					 // 노드스트림인 fist에 필터스트림(보조스트림)을 장착함.
					 // bost[필터스트림(보조스트림)]의 버퍼크기는 1024*1024byte 로 했다.
					 
					 
					 
					 byte[] dataArr = new byte[1024*1024];  // 1024*1024 byte == 1Mb
					 int inputLength = 0;
					 ...
					 // 닫을때는 보조(필터)스트림 부터 먼저 닫고, 그다음에 노드스트림을 닫는다.
					 bost.close();
					 fost.close(); 
